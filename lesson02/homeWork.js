...
alert (`Задание №0 \nтип переменной str - ${typeof str} \nтип переменной num - ${typeof num} \nтип переменной flag - ${typeof flag} \nтип переменной txt - ${typeof txt}`);
...
let s = parseInt(width) * parseInt(height);
...
let sizeOfCredit = '1 тыс.бел.руб.',
  interestRate = '10%',
  numberOfYears = 5,
  loanOverpayment;

loanOverpayment = (parseInt(sizeOfCredit) * 1000) * (parseInt(interestRate) / 100) * parseInt(numberOfYears);

alert (`Задание №4 \nПереплату по кредиту = ${loanOverpayment} бел.руб.`);
...
let date = new Date (2020, 6, 22, 18, 22, 35, 358);

let secondsPerWeek = 7 * 24 * 60 * 60;

alert (`Задание №5 \nСегодня ${date} и узнал что количество секунд в одной неделе составляет ${secondsPerWeek} c.`);
...
alert (`Задание №8 \nВремя выполнения задания ${hour}:${minute}:${second}`);
...
// Изменил на переменную 'num1' , потому что вверху уже есть такая переменная как 'num' 
let num1 = 10;
num1 = num1++;
num1 = ++num1;
num1 = num1--;

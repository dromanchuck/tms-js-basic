...
let monthlyPayment =
  (amount * (percent * Math.pow(1 + percent, period))) /
    (Math.pow(1 + percent), period) -
  1

let sumPayment = period * monthlyPayment - 1000
...
let secondsInWeek = 7 * 24 * 60 * 60
console.log(
  `Сегодня 16 января я узнал, что количество секунд в одной неделе составляет ${secondsInWeek}.`
)
...
const minNum = (firtsNum, secondNum) => {
...
console.log(
  `Знак числа x, y, z: ${signNumberX(-3)}, ${signNumberY(8)}, ${signNumberZ(
    -5
  )}`
)
...
const maxNum = (q, w, e, r, t) => {
  if (q > w && q > e && q > r && q > t) {
    ...
    return 'Есть число больше!'
  }
}

console.log(maxNum(100, 10, 20, 30, 40))
